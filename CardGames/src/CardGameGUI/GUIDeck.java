...
public class GUIDeck {
	...
	//Create a card
	public void paintCards(Graphics g, ArrayList<Card> Cards, int location) {
		int index = 0;
		for (Card c : Cards) {
			...
			} else if (c.shape.equalsIgnoreCase("Spades") || c.shape.equalsIgnoreCase("Clubs")) {
				g.setColor(Color.black);
			}else {
				g.setColor(Color.DARK_GRAY);
			}
		
			g.drawString(c.symbol, gridX+spacing+tCardW*index+rounding, gridY+spacing+cardH-rounding+location);
